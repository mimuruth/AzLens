...
The composer supports **dictation** (speech-to-text mic) and one-click **image generation** from the prompt, alongside file/image attachments:

![AzLens composer close-up showing the attach, dictate microphone, and generate-image buttons with a prompt typed in](docs/chat-ui-composer.png)
...
- **Image generation** — a composer button turns the prompt into an image via Azure OpenAI or OpenAI (DALL·E), inserting it into the chat. Backend at [/api/image](chat-ui/app/api/image/route.ts); prompts are moderated first.
...
- **Plan approval (human-in-the-loop)** — before running, the planner's proposed sub-tasks are shown for **review/editing** (change the agent, reword, add/remove steps) via [POST /api/orchestrate/plan](chat-ui/app/api/orchestrate/plan/route.ts); the approved plan is then executed.
